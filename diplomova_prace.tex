...
Problematickým rozdílem mezi programováním a malováním je účel výsledku. Obrazy jsou převážně na ozdobu, zatímco program má sloužit svému účelu, čímž jsou na něj kladeny úplně jiné nároky. Programátor dokáže vytvořit program, ovšem softwarový inženýr chápe smysl úmyslného omezení, které plyne z dodržování pravidel společně s \textit{best-practices}. Také ví, kdy pravidla a \textit{best-practices} obejít a co za následky z toho plyne.
...
\section{Technický dluh a jeho řešení}
...
Tento model nám pomáhá pochopit vznik a druhy technického dluhu, avšak je technický dluh opravdu takový problém?  
...
Clean Code a technický dluh jdou ruku v ruce. Jedna z příčin technického dluhu je právě zanedbání čitelnosti a udržitelnosti za cenu momentálního zrychlení vývoje. Dodržení, či alespoň snaha o Clean Code, nese sebou cenu dočasného zpomalení vývoje. Vývojář dočasně investuje více kongitivní aktivity do práce s kódem, aby ji mohl v budoucnu ušetřit.

V této kapitole si dovolím vyzdvihnout několik zásad, která mají dle mého názoru největší dopad na čitelnost a udržitelnost kódu.
...
\textit{Clean Code} ať už to jako koncept nebo jako kniha nemá být vnímána, ani v rámci v této práce, jako konkrétní body, které se v programu kontrolují podobně jako když se hledají běžné zranitelnosti (CVE). Jedná se spíše o mantru, která vede k hlubšímu porozumnění problematiky čitelnosti kódu. Každý vývojář dospěje do fáze, kdy chce vše dělat podle pravidel, aby následně zjistil, že je potřeba uvážit situace, kdy pravidla a doporučení více škodí, než pomáhají. Stejně tak \textit{Clean Code} není konceptem, který by měl být vštípán do hlavy začátečníkům, ale slouží jako horizontální rozšíření dovedností jednotlivce. Není tak důležité znění daných pravidel, ale jejich smysl a jaký problém se snaží řešit. V takový moment je možné uvažovat, která pravidla nasadit, a naopak která vynechat, pokud nás daný problém netrápí. Stejně jako mluvené slovo, jazyky jsou expresivní záleží na jednotlivci, komu je jeho slovo určeno a jak se chce vyjadřovat.

\section{Zásady distribuovaného programování}

\section{Zásady resilientní komunikace}
